...
# --- Main app (only reached if username is set) ---
...
tab1, tab2 = st.tabs(["📤 Upload New", "📊 History"])
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        # Create a unique identifier for this specific uploaded file
        file_signature = f"{uploaded_file.name}_{uploaded_file.size}"

        # Only analyze and save if this is a new file we haven't processed yet
        ...
        if result is not None:
            if isinstance(result, dict) and result.get("error") == "no_financial_data":
                ...
                for transaction in result:
                   ...
                   # Save this transaction to the database (only if it's a new upload)
...
with tab2:
    ...
    history = get_user_transactions(st.session_state.username)

    if not history:
        st.info("No transactions yet. Upload a receipt to get started!")
    else:
        st.metric("Total Transactions", len(history))
        st.markdown("---")

        for t in history:
            icon = get_icon(t.get("category", "other"))
            card_html = f"""
            <div class="transaction-card">
                <div class="transaction-title">{icon} {t['merchant']} — {t['total']}</div>
